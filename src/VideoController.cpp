...
{
    this->BottomCharacters = _bottomCharacters;
...
void IRAM_ATTR drawScanline(void* arg, uint8_t* dest, int scanLine)
{
    ...
    if (scanLine >= (BORDER_HEIGHT + BK_HEIGHT) * 2)
    {
        ...
        else
        {
            ...
            uint8_t* characters = controller->BottomCharacters + startCoord;
            uint32_t* palette = controller->PaletteText;
            uint32_t* dest32 = (uint32_t*)dest;
            uint32_t* lastDest = dest32 + (SCREEN_WIDTH * 2);
            uint8_t* fontData = (uint8_t*)fabgl::FONT_8x9.data + fontRow;
            uint8_t character;
            uint8_t fontPixels;
            ...
            do
            {
                ...
                dest32[0] = palette[fontPixels >> 4];
                dest32[1] = palette[fontPixels & 0x0F];
